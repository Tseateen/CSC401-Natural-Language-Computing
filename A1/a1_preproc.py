        ...
        for sent in doc.sents:
            for token in sent:
                if token.text.isupper():
                    s += token.lemma_.upper() + "/" + token.pos_.upper()
                else:
                    s += token.lemma_ + "/" + token.pos_.upper()
                s += " "
            s += "\n"
        modComm = s
...
def main(args):
    ...
    for subdir, dirs, files in os.walk(indir):
        for file in files:
            ...
            length = len(data)
            index = args.ID[0] % length
            for i in range(index, index + args.max):
                j = json.loads(data[i % length])
                a = {}
                a["id"] = j["id"]
                a["body"] = preproc1(j["body"])
                a["cat"] = file
                allOutput.append(a.copy())
