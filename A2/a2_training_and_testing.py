...
def train_for_epoch(
    ...
    # the loop.
    lfn = torch.nn.CrossEntropyLoss(ignore_index=model.source_pad_id)
    lst = []
    ...
    for F, F_lens, E in tqdm(dataloader):
        ...
        log = torch.flatten(log, start_dim=0, end_dim=1)
        ...
        optimizer.step()
        lst.append(float(loss))
        del F, F_lens, E, log, loss
...
def compute_batch_total_bleu(
    ...
    # of numbers
    total = 0.0
    er_list = E_ref.transpose(0, 1).tolist()
    ec_list = E_cand.transpose(0, 1).tolist()
    for i in range(E_ref.shape[1]):
        r_list = []
        c_list = []
        for item in er_list[i]:
            if int(item) != target_eos and int(item) != target_sos:
                r_list.append(item)
        for item in ec_list[i]:
            if int(item) != target_eos and int(item) != target_sos:
                c_list.append(item)
        total += a2_bleu_score.BLEU_score(r_list, c_list, 4)
...
def compute_average_bleu_over_dataset(
    ...
    return 0.0 if count == 0 else float(score / count)
